...
private:
	...
	// Note: vector<signed char> is used instead of vector<ArcState> for efficiency reasons
	...
	// Node and arc data
	CostVector _cost, _pi;
	ValueVector _supply, _flow;

	// Data for storing the spanning tree structure
	IntVector _parent, _thread, _rev_thread, _succ_num, _last_succ, _dirty_revs;
	ArcVector _pred;
	BoolVector _forward;
	StateVector _state;
	ArcsType _root;
...
public:
	...
	NetworkSimplexSimple & supplyMap(const SupplyMap * map1, int n1, const SupplyMap * map2, int n2) {
		Node n; _graph.first(n);
		for (; n != INVALIDNODE; _graph.next(n)) {
			if (n < n1) _supply[_node_id(n)] = map1[n];
			else _supply[_node_id(n)] = map2[n - n1];
		}
		return *this;
	}
	template<typename SupplyMap>
	NetworkSimplexSimple & supplyMapAll(SupplyMap val1, int n1, SupplyMap val2, int n2) {
		Node n; _graph.first(n);
		for (; n != INVALIDNODE; _graph.next(n)) {
			if (n < n1) _supply[_node_id(n)] = val1;
			else _supply[_node_id(n)] = val2;
		}
		return *this;
	}
	...
	NetworkSimplexSimple& reset() {
		...
		if (_arc_mixing && _node_num > 1) {
			...
			for (Arc a = 0; a <= _graph.maxArcId(); a++) {
...
private:
	...
	void findJoinNode() {
		...
		while (u != v) {
			if (_succ_num[u] < _succ_num[v]) u = _parent[u];
			else v = _parent[v];
		}
		join = u;
	}
